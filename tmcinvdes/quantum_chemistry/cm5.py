...
def GetLogFile(lines, pt_df, rad_df):
    ...
    rev_start_idx = None  # Just a dummy value
    rev_start_idx_hirsh = None  # Just a dummy value
    for i, line in enumerate(reversed(lines)):
        if "CARTESIAN COORDINATES (ANGSTROEM)" in line:
            rev_start_idx = i - 1
        if "HIRSHFELD ANALYSIS" in line:
            rev_start_idx_hirsh = i - 6
        if rev_start_idx and rev_start_idx_hirsh:
            break

    for line in lines[-rev_start_idx : -rev_start_idx + num_atoms]:
        xyz_data.append(line.strip().split())

    for line in lines[-rev_start_idx_hirsh : -rev_start_idx_hirsh + num_atoms]:
        charge_data.append(line.strip().split())

    hirCharges = pd.DataFrame(charge_data, columns=["N", "ATOM", "QHir", "Spin"])
    hirCharges[["N", "QHir", "Spin"]] = hirCharges[["N", "QHir", "Spin"]].apply(
        pd.to_numeric
    )
    hirCharges = hirCharges[["N", "QHir"]]
    xyzcoos = pd.DataFrame(xyz_data, columns=["ATOM", "X", "Y", "Z"])
...
def xyz_prep(df):
    ...
    opdb.close()
    os.system("obabel -ixyz inp_orca.xyz -omol inp_orca.mol")
    hmol = Chem.MolFromMolFile("nh3.mol", removeHs=False)
    return hmol
...
def LoadModel():
    ...
    with open(
        os.path.abspath(os.path.join(os.path.dirname(__file__), "cm5pars.json")), "r"
    ) as tweetfile:
...
def get_cm5(lines):
    a0, rd, pt = LoadModel()
    data = GetLogFile(lines, pt, rd)
    qcm5 = HirshfeldToCM5(data, a0)
    qcm5_dict = qcm5.to_dict()
