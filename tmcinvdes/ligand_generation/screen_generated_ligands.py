...
def parse_args(arg_list: list = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="""Input directory with the raw, unprocessed files of generated ligands as encoded
        SMILES.""",
    )
    parser.add_argument(
        "--output_dir",
...
# Simple regex functions to count substring patterns in encoded SMILES as strings:
...
def process_encoded_smiles_monodentate(
    encoded_smilesx: list,
    atom_counting_mode: str,
    count_atoms_fun: Callable,
    process_substitutions_fun: Callable,
) -> dict:
    """Process encoded SMILES strings into Mol objects and decoded SMILES, or
    else discard them. Monodentate version.

    Args:
        encoded_smilesx (list): list of the validated subset of encoded SMILES, Mol objects and
        connection atoms.
        ...
        the encoded SMILES encoding.

    Returns:
        dict: a map of the successfully processed ligands, with decoded SMILES as key for easy
        comparison between generated and training ligands.
    """
    processed_dict = {}

    for smile, mol, connection_id in encoded_smilesx:
        atom = mol.GetAtomWithIdx(connection_id)
        neighbors = atom.GetNeighbors()
        ...
        try:
            ...
            if isinstance(new_connection_ids, int):  # Always true for monodentates.
                new_connection_ids = [new_connection_ids]
            decoded_smiles, hless_mapped_ids = get_smiles_donor_id(new_mol)
            try:
                decoded_connection_ids = [
                    hless_mapped_ids[x] for x in new_connection_ids
                ]
            except Exception:
                if connection_id == 1:
                    decoded_connection_ids = [connection_id - 1]
                else:
                    print(smile)
                    print(connection_id)
                    # In this case there may be right-sided encoding.
                    print(
                        f"Maybe try connection ID as: {len([a for a in mol.GetAtoms()])-1}"
                    )
                    sys.exit()
            atom_count = count_atoms_fun(mol)
            processed_dict[decoded_smiles] = {
                "mol": new_mol,
                "Encoded SMILES": smile,
                "Connection IDs": decoded_connection_ids,
...
def process_encoded_smiles_bidentate(
    encoded_smilesx: list,
    atom_counting_mode: str,
    count_atoms_fun: Callable,
    process_substitutions_fun: Callable,
    ...
    Args:
        encoded_smilesx (list): list of encoded SMILES strings.
        atom_counting_mode (str): the mode of counting atoms per ligand.
        count_atoms_fun (Callable): the function used to count atoms.
        process_substitutions_fun (Callable): the function used to process the substitutions from
        the SMILES encoding.
    ...
    for smile in encoded_smilesx:
        ...
        try:
            ...
            decoded_smiles, hless_mapped_ids = get_smiles_donor_id(mol)
            try:
                decoded_connection_ids = [hless_mapped_ids[x] for x in connection_ids]
            except Exception:
                continue
            atom_count = count_atoms_fun(mol)
            # Mark for deletion those ligands where the connection IDs are not unambiguous:
            if decoded_smiles in processed_dict:
                if (
                    processed_dict[decoded_smiles]["Connection IDs"]
                    != decoded_connection_ids
                ):
                    processed_dict[decoded_smiles] = {
                        "mol": None,
...
def validate_encoded_smiles_monodentate(encoded_smilesx: list) -> list:
    """Validate encoded SMILES that represent monodentate ligands.

    This removes encoded SMILES which fall outside the expected form of generated ligands, at a
    string level of testing.

    Args:
        encoded_smilesx (list): the encoded SMILES to validate.

    Returns:
        list: the validated subset of encoded SMILES, Mol objects and connection atoms.
    """
    encoded_smilesx.sort(key=lambda x: Chem.MolFromSmiles(x).GetNumAtoms())
    smiles_mols = [
        tuple([x, Chem.MolFromSmiles(x)])
        for x in encoded_smilesx
...
def validate_encoded_smiles_bidentate(encoded_smilesx: list) -> list:
    """Validate encoded SMILES that represent bidentate ligands.

    This removes encoded SMILES which fall outside the expected form of generated ligands, at a
    text string level of testing.

    Args:
        encoded_smilesx (list): the encoded SMILES to validate.

    Returns:
        list: the validated subset of encoded SMILES.
    """
    validated_encoded_smilesx = []
    for smile in encoded_smilesx:
        ...
        if count_ir == 1 and count_dat == 2 and count_bid == 1:
            validated_encoded_smilesx.append(smile)
    return validated_encoded_smilesx
...
def screen_generated_ligands(
    ...
    denticity: str,
    atom_counting_mode: str,
    validate_encoded_smiles_fun: Callable,
    count_atoms_fun: Callable,
    process_substitutions_fun: Callable,
) -> pd.DataFrame:
    ...
    # Remove generated encoded SMILES that are verbatim string matches of training set instances:
    unique_generated = [x for x in unique_generated if x not in unique_train]
    validated_generated = validate_encoded_smiles_fun(unique_generated)
    # No need to validate the train set of Encoded SMILES.
    if denticity == "monodentate":  # Shared code for monodentates diverges
        processed_generated = process_encoded_smiles_monodentate(
            validated_generated,
            atom_counting_mode,
            count_atoms_fun,
        ...
        processed_generated = process_encoded_smiles_bidentate(
            validated_generated,
            atom_counting_mode,
            count_atoms_fun,
            process_substitutions_fun,
        ...
        processed_train = process_encoded_smiles_bidentate(
        ...
        # Remove generated encoded SMILES that are structural matches of training set instances:
        for train_decoded_smiles in processed_train.keys():
            if train_decoded_smiles in processed_generated:
                del processed_generated[train_decoded_smiles]
    ...
    for smiles, vals in processed_generated.items():
        ...
        data_dict["Decoded SMILES"].append(smiles)
        data_dict["Connection IDs"].append(sorted(vals["Connection IDs"]))
        data_dict["Encoded SMILES"].append(vals["Encoded SMILES"])
...
if __name__ == "__main__":
    ...
    if denticity == "monodentate":
        ...
        validate_encoded_smiles_fun = validate_encoded_smiles_monodentate
        ...
        validate_encoded_smiles_fun = validate_encoded_smiles_bidentate
        count_atoms_fun = count_atoms_hfull
        process_substitutions_fun = process_substitute_attachment_points_bidentate

    # These files should have no column header, each is just a single column of encoded SMILES.
